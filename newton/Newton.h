...
extern "C" {
	...
	typedef struct NewtonChamferCylinderParam NewtonTaparedCylinderParam;
	...
	struct NewtonUserMeshCollisionCollideDesc
	{
		...
		dFloat m_boxDistanceTravel[4];				// max distance that box bpxP0 and boxP1 can tavle on thsi step, used this for continue collision mode
		...
		int* m_faceVertexIndex;                 	// the application should set here the pointer index array for each vertex on a face.
                                                	...
                                                	// I0, I1, I2, .. are the indices to the vertex, relative to m_vertex
		                                        	// M is the index to the material sub shape if
													// N in the index to the vertex normal relative to m_vertex
													// E0, E1, E2, ... are the indices, relative m_vertex, of the the face normal that is shared to that face edge, 
													// when the edge does no share a face normal then the edge index is set to N, the face normal    
													// A is the estimate larger of the large diagonal of the face
	};
	...
	typedef void (*NewtonWorldListenerCallback) (const NewtonWorld* const world, void* const listenerUserData, dFloat timestep);
...
//	NEWTON_API void NewtonWorldForEachBodyDo (const NewtonWorld* const newtonWorld, NewtonBodyIterator callback);
	...
	NEWTON_API void NewtonWorldSetPreUpdateListenerCallback (const NewtonWorld* const newtonWorld, NewtonWorldListenerCallback listener, void* const listenerUserData);
	NEWTON_API NewtonWorldListenerCallback NewtonWorldGetPreUpdateListenerCallback (const NewtonWorld* const newtonWorld);

	NEWTON_API void NewtonWorldSetPostUpdateListenerCallback (const NewtonWorld* const newtonWorld, NewtonWorldListenerCallback listener, void* const listenerUserData);
	NEWTON_API NewtonWorldListenerCallback NewtonWorldGetPostUpdateListenerCallback (const NewtonWorld* const newtonWorld);
...
//	NEWTON_API dFloat NewtonCollisionGetMaxBreakImpactImpulse(const NewtonCollision* const convexHullCollision);
	...
	NEWTON_API void NewtonCompoundCollisionSetSubCollisionMatrix (NewtonCollision* const compoundCollision, void* const node, dFloat* const matrix);	
	...
	NEWTON_API void* NewtonCompoundCollisionGetNextNode (NewtonCollision* const compoundCollision, void* const node);

	NEWTON_API void* NewtonCompoundCollisionGetNodeByIndex (NewtonCollision* const compoundCollision, int index);
	NEWTON_API int NewtonCompoundCollisionGetNodeIndex (NewtonCollision* const compoundCollision, void* const node);
	NEWTON_API NewtonCollision* NewtonCompoundCollisionGetCollisionFromNode (NewtonCollision* const compoundCollision, void* const node);
...
*/
	...
	NEWTON_API NewtonCollision* NewtonCreateUserMeshCollision (const NewtonWorld* const newtonWorld, const dFloat* const minBox, 
		...
		NewtonOnUserCollisionSerializationCallback serializeCallback, int shapeID);
	...
	NEWTON_API int NewtonCollisionCollide (const NewtonWorld* const newtonWorld, int maxSize,
		...
		dFloat* const contacts, dFloat* const normals, dFloat* const penetration, int* const attribute,  
		int threadIndex);
	...
	NEWTON_API void  NewtonBodySetCentreOfMass___ (const NewtonBody* const body, const dFloat* const com);
	NEWTON_API void  NewtonBodySetMassMatrix___ (const NewtonBody* const body, dFloat mass, dFloat Ixx, dFloat Iyy, dFloat Izz);
